...
import {
  ...
  starterPrompts,
  type StarterPromptCategoryId,
} from "~/features/prompts/starter-prompts";

describe("starter prompts", () => {
  it("keeps the curated beta prompt inventory stable", () => {
    expect(starterPromptCategories.map((category) => category.id)).toEqual([
      "casual",
      "deep",
      "funny",
      "friends",
    ...
    expect(starterPrompts).toHaveLength(60);

    for (const category of starterPromptCategories) {
      expect(category.prompts.map((prompt) => prompt.text)).toEqual(
        expectedPromptTextByCategory[category.id],
      );
    }
  });

  it("finds prompts by stable id", () => {
    expect(findStarterPrompt("work_school-01")).toMatchObject({
      id: "work_school-01",
      categoryId: "work_school",
      text: "What project or class is teaching you the most right now?",
    });
    expect(findStarterPrompt("missing")).toBeUndefined();
  });
});

const expectedPromptTextByCategory = {
  casual: [
